//
//  UniqueKeyObjectPoolTests.swift
//
//
...
final class UniqueKeyObjectPoolTests: XCTestCase {
    func testObjectPool() throws {
        let objectPool = UniqueKeyObjectPool<Int, MocObject> { key in
    ...
    func testObjectPoolTheSameRef() throws {
        let objectPool = UniqueKeyObjectPool<Int, MocObject>(maxSize: 3) { key in
            MocObject(intValue: key, stringValue: "\(key)")
        }
